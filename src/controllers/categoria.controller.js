...
const getFormato = async(request, response) => {
    //CAMBIAR STOREPROCEDURE
        try{
            ...
            if(parseInt(nuevo) == 0)
            { 
                qString = "SELECT CONCAT(CAST(envase_disponible AS CHAR CHARACTER SET utf8), ' $', FORMAT(ROUND((CAST(PRECIO_NUEVO AS CHAR CHARACTER SET utf8) / 1.18)),0, 'de_DE'),  '+IVA') as name from lubricentro_productos where NOMBREPRODUCTO = ?"; 
            } else {
    ...
    const getformatoOferta = async(request, response) => {
        //CAMBIAR STOREPROCEDURE
            try{
                ...
                if(parseInt(nuevo) == 0)
                { 
                    qString = "SELECT CONCAT(CAST(envase_disponible AS CHAR CHARACTER SET utf8), ' $', FORMAT(ROUND((CAST(PRECIO_NUEVO AS CHAR CHARACTER SET utf8) / 1.18)),0, 'de_DE'),  '+IVA') as name from lubricentro_productos where NOMBREPRODUCTO = ? AND CodPromo LIKE CONCAT('%', '" + codigo + "' , '%')"; 
                } else {
